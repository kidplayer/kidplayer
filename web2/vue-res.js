...
new Vue({
  ...
  data: {
    ...
    gaCode:
      '<ins class="adsbygoogle" style="display:block;" data-ad-client="ca-pub-2341115656300335" data-ad-slot="2960734406" data-ad-format="horizontal"></ins><script>(adsbygoogle = window.adsbygoogle || []).push({});</script>',
  },
  watch: {
    link: function(t, e) {
      0 < t.length
    ...
    html: function(t, e) {
      0 < t.length ? (this.showClearBtn = !0) : (this.showClearBtn = !1);
    },
  },
  methods: {
    gaga: function(t, e) {
      ga("send", "event", t, t + "-" + (e + 1));
    },
    ...
    submit: function(t) {
      if (!this.submitBtnClass.disabled) {
        if ((this.removeLastResult(), "shipinhao" === site)) {
          if ("" === this.html || this.html.length < 200)
            return void (this.errorTip = "请先将源码正确的粘贴到上面的输入框");
          this.link =
            "http://finder.video.qq.com?html=" + Base64.encode(this.html);
        }
        if ("" !== this.link) {
          var e = this.link.lastIndexOf("http://");
          if (-1 !== (e = -1 === e ? this.link.lastIndexOf("https://") : e)) {
            this.link = this.link.substr(e);
            var n = this.link.indexOf(" ");
            -1 !== n && (this.link = this.link.substring(0, n));
            this.link.length < 16 ||
            !/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z]{2,5}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g.test(
              this.link
            )
              ...
              : ((this.errorTip = ""),
                i(this.link)
                  ? (this.isMP4File = !0)
                  : r.contains(l(this.link))
                  ? this.unShortUrlAndParseVideo()
                  : this.parseVideo());
          } else this.errorTip = "请输入正确的视频链接";
        } else this.errorTip = "请先将视频链接粘贴到上面的输入框";
      }
    },
      ...
      if (
        ...
        $.ajax({
          ...
          error: function(t, e, n) {
            (r.submitBtnClass.disabled = !1),
              h() && "error" === e && -2147024891 === n.number
                ? (r.errorTip =
                    "由于您IE浏览器的安全设置，本站部分功能无法正常使用，请更改IE浏览器安全设置或更换为WebKit内核浏览器访问本站获得最佳上网体验。IE浏览器设置方法: 打开IE浏览器，依次选择右上角工具—Internet选项-安全-自定义级别-其它-通过域访问数据源，设置为启用即可。")
                : (r.errorTip = "处理失败,请检查您的网络连接后重试!");
          },
        });
      }
    },
    init: function() {
      !(function e() {
        h() &&
          $("#app .row .col-md-12").prepend(
            '<div style="text-align: center;" class="alert alert-danger" role="alert">发现您正在使用IE内核浏览器，部分功能可能无法正常使用，建议使用WebKit内核浏览器访问本站获得最佳上网体验。<a target="_blank" href="http://www.jianshu.com/p/a70521aba2b3">详情>></a></div>'
          );
      })(),
        -1 < window.navigator.userAgent.indexOf("MicroMessenger")
          ? (this.browserTip =
              "因微信内置浏览器不支持下载文件, 请在iiiLab推荐的浏览器中打开本站: " +
              window.location.origin)
          : (this.downloadTip = !0);
      var t = "?link=";
      if (
        -1 !== location.search.indexOf(t) &&
        "" !== location.search.substr(t.length)
      )
        return (
          (this.link = decodeURI(location.search.substr(t.length))),
          window.history &&
      ...
      "undefined" != typeof Storage &&
        (null != localStorage.getItem("time") &&
        36e5 < new Date().getTime() - localStorage.getItem("time")
          ? this.removeLastResult()
          : (null != localStorage.getItem("link") &&
              (this.link = localStorage.getItem("link")),
            null != localStorage.getItem("requestResult") &&
    ...
    initSponsor: function() {
      ...
      $.ajax({
        ...
        success: function(t) {
          t.succ &&
            ((e.sponsor.text = t.data.text || []),
            (e.sponsor.img = t.data.img || []));
        },
        error: function() {},
        complete: function() {
          e.init(), 0 === e.sponsor.img.length && $("#ga-code").html(e.gaCode);
        },
      }),
        "undefined" != typeof goldSponsorFlag &&
          goldSponsorFlag &&
          $.ajax({
            type: "POST",
            url: a + "/sponsor/getSponsor",
            xhrFields: {
              withCredentials: !0,
            },
            crossDomain: !0,
            ...
            success: function(t) {
              t.succ &&
                ((e.goldSponsor.text = t.data.text || []),
                (e.goldSponsor.img = t.data.img || []));
            },
            error: function() {},
            complete: function() {},
          });
    ...
    redirect: function(t) {
      var e = location.protocol + "//" + t + "/?link=" + encodeURI(this.link);
...
try {
  window.console &&
    window.console.log &&
    console.log(
      "iiiLab视频解析接口开发和管理指南：http://www.iiilab.com/article/60"
    );
} catch (f) {}
